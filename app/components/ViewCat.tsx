...
interface Props {
  sortedTodos: Todo[];
  stylizedCategories: string;
}

export default function ViewCat({ sortedTodos, stylizedCategories }: Props) {
  const headerClasses =
        ...
        <tbody className="border-x-[5px] border-t-0 border-b-[5px] border-solid border-purple-500 p-5">
          {sortedTodos.length == 0 ? (
            ...
            sortedTodos.reverse().map((todo) => (
              ...
              >
                <td className={bodyClasses}>{stylizeStatuses(todo.status)}</td>
